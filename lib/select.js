...
Select.prototype._exec = function (current, row, p) {
...
Select.prototype._fork = function (current, row) {
    var sub = new Select(this._select, function () {});
    sub.input.write(row);
    this._submatches.push(sub);
    this.emit('fork', sub);
    this.emit('match', sub);
};
...
Select.prototype.createStream = function () {
    ...
    this.input.pipe(through.obj(
        function (row, enc, next) { r.push(row); next() },
